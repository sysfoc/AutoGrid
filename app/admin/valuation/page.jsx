      ...
      <div className="relative mx-auto w-full max-w-md rounded-xl bg-white shadow-xl">
        <div className="p-4 sm:p-6">
          ...
          <p className="mb-6 text-sm text-gray-600 sm:text-base">
            Are you sure you want to delete this valuation request? This will
...
const AdminValuationPage = () => {
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setReplyMessage(
        "Reply sent successfully and customer notified via email!",
      );
      setAdminReply("");
      setEstimatedValue("");
  ...
  const getValuationTypeBadge = (type) => {
    const typeConfig = {
      Selling: "bg-green-100 text-app-bg border-green-200",
    ...
    return (
      <span
        className={`inline-flex items-center rounded-full border px-2 py-1 text-xs font-medium ${
          typeConfig[type] || "border-gray-200 bg-gray-100 text-gray-800"
        }`}
      >
        {type}
      </span>
  ...
  const handleDeleteValuation = async () => {
    ...
    try {
      ...
      setValuations((prev) =>
        prev.filter((val) => val._id !== valuationToDelete),
      );
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            <div>
              ...
              <p className="mt-2 text-sm text-gray-600 sm:text-base">
                Manage and respond to car valuation requests
              </p>
            </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all hover:scale-[1.015] hover:shadow-md sm:p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-app-text sm:text-3xl">
                  {stats.total}
                </p>
              </div>
              <div className="rounded-xl bg-blue-100 p-2 sm:p-3">
                <Car className="h-5 w-5 text-blue-600 sm:h-6 sm:w-6" />
              </div>
            </div>
          </div>
          ...
          {currentValuations.length === 0 ? (
            <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm sm:p-12">
              <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-gray-100 sm:h-16 sm:w-16">
                <Car className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
              </div>
              <h3 className="mb-2 text-base font-semibold text-app-text sm:text-lg">
                No valuations found
              </h3>
                ...
                <div className="p-4 sm:p-6">
                  ...
                  <div className="mb-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-app-button to-red-700 sm:h-12 sm:w-12">
                    ...
                    <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-4">
                      ...
                      {valuation.repliedAt && (
                        <span className="flex items-center gap-1">
                          <CheckCircle size={12} />
                          Replied{" "}
                          {new Date(valuation.repliedAt).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                    <span className="text-xs">
                      ID: {valuation._id.slice(-8)}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-6 flex flex-col items-center justify-between gap-4 rounded-2xl border border-gray-200 bg-white px-4 py-4 shadow-sm sm:mt-8 sm:flex-row sm:px-6">
            <div className="flex items-center gap-2 text-xs text-gray-600 sm:text-sm">
              <span>
                ...
                {Math.min(endIndex, filteredValuations.length)} of{" "}
                {filteredValuations.length} valuations
              </span>
            </div>
        ...
        >
          <ModalHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-red-100 sm:h-10 sm:w-10">
                <Reply className="h-4 w-4 text-app-button sm:h-5 sm:w-5" />
              </div>
              <div>
                <h3 className="text-base font-semibold text-app-text sm:text-lg">
                  {selectedValuation?.status === "pending"
                    ? "Reply to Valuation"
                    : "Edit Reply"}
                </h3>
          ...
          <ModalBody className="pb-6">
            <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:mb-6 sm:p-4">
              ...
              <div className="grid grid-cols-1 gap-4 text-xs text-gray-600 sm:grid-cols-2 sm:text-sm">
                <div>
                  <span className="font-medium">Vehicle:</span>{" "}
                  {selectedValuation?.make} {selectedValuation?.model}
                </div>
                <div>
            ...
            <div className="mb-4 sm:mb-6">
              ...
              <p className="mt-1 text-xs text-gray-500">
                📧 The customer will be automatically notified via email when
                you send this reply.
              </p>
            </div>
            {replyMessage && (
              <div
                className={`mb-4 rounded-lg p-3 text-xs sm:text-sm ${
                  replyMessage.includes("successfully")
                    ? "text-app-bg border border-green-200 bg-green-50"
            ...
            <div className="flex flex-col justify-end gap-3 pb-10 sm:flex-row">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-app-button px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-app-button-hover disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
              >
                {replying ? (
                  <>
                    <Spinner size="sm" />
              ...
              <>
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 sm:p-4">
                    <h4 className="mb-2 text-sm font-medium text-app-text sm:text-base">
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-3 sm:p-4">
                  <h4 className="mb-3 text-sm font-medium text-app-text sm:text-base">
                    Vehicle Information
                  </h4>
                    ...
                    <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-2">
                      ...
                      {selectedValuation.repliedBy && (
                        <span className="text-app-bg text-xs">
                          by {selectedValuation.repliedBy}
                        </span>
                      )}
                    </div>
                    {selectedValuation.estimatedValue && (
                      <div className="mb-2">
                        <span className="text-app-bg text-xs font-medium sm:text-sm">
                          Estimated Value:
                        </span>
                        <span className="text-app-bg ml-1 text-xs sm:text-sm">
                          {selectedValuation.estimatedValue}
                        </span>
                      </div>
                    )}
