  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black bg-opacity-50 p-2 sm:p-4">
      ...
      >
        <button
          onClick={onClose}
          className="absolute right-2 top-2 z-50 cursor-pointer text-black hover:text-gray-500 sm:right-4 sm:top-4"
        >
          <MdCancel size={24} className="sm:h-8 sm:w-8" />
        </button>
        {children}
      </div>
    </div>
...
const ModalHeader = ({ children }) => (
  <div className="sticky top-0 z-10 rounded-t-2xl border-b border-gray-200 bg-white p-3 sm:p-6">
    {children}
  </div>
);
...
const ModalBody = ({ children, className = "" }) => (
  <div
    className={`max-h-[70vh] overflow-y-auto p-3 sm:max-h-[80vh] sm:p-6 ${className}`}
  >
    {children}
  </div>
);
...
const Spinner = ({ size = "md" }) => {
  ...
  return (
    <div
      className={`inline-block ${sizeClasses[size]} animate-spin rounded-full border-solid border-app-bg border-t-transparent`}
    />
  );
};
      ...
      <div className="relative mx-auto w-full max-w-md rounded-xl bg-white shadow-xl">
        <div className="p-4 sm:p-6">
          ...
          <p className="mb-6 text-sm text-gray-600 sm:text-base">
            Are you sure you want to delete this rate request? This will
          ...
          <div className="flex flex-col justify-end gap-3 sm:flex-row">
            <button
              onClick={onClose}
              disabled={loading}
              className="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-50 sm:w-auto"
            >
              Cancel
            </button>
            <button
              onClick={onConfirm}
              disabled={loading}
              className="flex w-full items-center justify-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700 disabled:opacity-50 sm:w-auto"
            >
              {loading ? (
                <>
                  <Spinner size="sm" />
...
const AdminRateRequestsPage = () => {
  ...
  const getStatusBadge = (status) => {
    ...
    const statusConfig = {
      ...
      answered: {
        color: "bg-green-50 text-app-bg border-app-border",
        icon: CheckCircle,
      },
    };
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-xs font-semibold sm:px-3 sm:py-1.5 ${config.color}`}
      >
  ...
  const handleDeleteRequest = async () => {
    ...
    try {
      ...
      setRateRequests((prev) =>
        prev.filter((req) => req._id !== requestToDelete),
      );
        ...
        <div className="mx-auto max-w-7xl">
          <div className="flex h-64 items-center justify-center">
            <div className="text-center">
              <div className="mx-auto mb-4 h-12 w-12 animate-spin rounded-full border-b-2 border-app-bg"></div>
              <span className="text-base font-medium text-slate-600 sm:text-lg">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-100">
      <div className="mx-auto max-w-7xl px-3 py-4 sm:px-4 sm:py-8 lg:px-8">
          ...
          <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-2 rounded-lg border border-gray-300 bg-app-bg px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-app-hover sm:px-4"
              >
                <RefreshCw size={16} />
        ...
        <div className="mb-6 grid grid-cols-1 gap-4 sm:mb-8 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3">
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all hover:scale-[1.015] hover:shadow-md sm:p-6">
            <div className="flex items-center justify-between">
              ...
              <div className="rounded-xl bg-green-100 p-2 sm:p-3">
                <Calculator className="h-5 w-5 text-app-bg sm:h-6 sm:w-6" />
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all hover:scale-[1.015] hover:shadow-md sm:col-span-2 sm:p-6 lg:col-span-1">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-500">Answered</p>
                <p className="text-2xl font-bold text-app-bg sm:text-3xl">
                  {stats.answered}
                </p>
              </div>
              <div className="rounded-xl bg-green-100 p-2 sm:p-3">
                <CheckCircle className="h-5 w-5 text-app-bg sm:h-6 sm:w-6" />
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              <div className="relative max-w-md flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-gray-300 py-2 pl-10 pr-4 text-sm focus:border-transparent focus:ring-2 focus:ring-app-bg"
                />
              </div>
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value)}
                className="rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-transparent focus:ring-2 focus:ring-app-bg sm:w-auto"
              >
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-xs font-medium transition-all duration-200 sm:text-sm ${
                    filter === tab.key
                      ? "border-2 border-app-border bg-green-100 text-app-bg"
          ...
          {currentRequests.length === 0 ? (
            <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm sm:p-12">
              <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-gray-100 sm:h-16 sm:w-16">
                <Calculator className="h-6 w-6 text-gray-400 sm:h-8 sm:w-8" />
              </div>
                ...
                <div className="p-4 sm:p-6">
                  ...
                  <div className="mb-4 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex items-center gap-3 sm:gap-4">
                      <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-app-bg to-app-border sm:h-12 sm:w-12">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500 sm:text-sm">
                          <Mail
                            size={12}
                            className="flex-shrink-0 sm:h-3.5 sm:w-3.5"
                          />
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          className="flex items-center justify-center gap-1 rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-50 sm:gap-2 sm:px-3 sm:py-2 sm:text-sm"
                        >
                        ...
                        <button
                          ...
                          className={`flex items-center justify-center gap-1 rounded-lg px-2 py-1.5 text-xs font-medium transition-colors sm:gap-2 sm:px-3 sm:py-2 sm:text-sm ${
                            request.status === "pending"
                              ? "bg-app-bg text-white hover:bg-app-hover"
                              : "bg-app-bg text-white hover:bg-app-hover"
                          }`}
                        >
                          <Reply size={12} className="sm:h-3.5 sm:w-3.5" />
                        ...
                        <button
                          ...
                          className="flex items-center justify-center gap-1 rounded-lg border border-red-300 bg-white px-2 py-1.5 text-xs font-medium text-red-600 transition-colors hover:bg-red-50 sm:gap-2 sm:px-3 sm:py-2 sm:text-sm"
                        >
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                      <div>
                        ...
                        <p className="text-sm font-semibold">
                          ${request.calculationData?.carPrice?.toLocaleString()}
                        </p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold">
                          {request.calculationData?.loanTerm} years
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="mt-6 flex flex-col items-center justify-between gap-4 rounded-2xl border border-gray-200 bg-white px-4 py-4 shadow-sm sm:mt-8 sm:flex-row sm:px-6">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1 rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-xs font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-50 sm:px-3 sm:py-2 sm:text-sm"
              >
          ...
          <ModalHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-green-100 sm:h-10 sm:w-10">
                <Reply className="h-4 w-4 text-app-bg sm:h-5 sm:w-5" />
              </div>
              <div>
                <h3 className="text-base font-semibold text-gray-900 sm:text-lg">
                  {selectedRequest?.status === "pending"
                    ? "Reply to Rate Request"
                    : "Edit Reply"}
                </h3>
          ...
          <ModalBody>
            <div className="mb-4 rounded-lg border border-gray-200 bg-gray-50 p-3 sm:mb-6 sm:p-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-xs sm:text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 font-medium">
                    {selectedRequest?.calculationData?.carYear}
                  </span>
                </div>
                <div>
            ...
            <div className="mb-4 sm:mb-6">
              ...
              <textarea
                ...
                rows={6}
                className="w-full rounded-lg border border-gray-300 p-3 text-sm focus:border-app-bg focus:ring-1 focus:ring-app-bg"
              ...
              <div
                className={`mb-4 rounded-lg p-3 text-xs sm:text-sm ${
                  replyMessage.includes("successfully")
                    ? "border border-green-200 bg-green-50 text-app-bg"
            ...
            <div className="flex flex-col justify-end gap-3 sm:flex-row">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-app-bg px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-app-hover disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
              >
                {replying ? (
                  <>
                    <Spinner size="sm" />
          ...
          <ModalHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 sm:h-10 sm:w-10">
                <Eye className="h-4 w-4 text-gray-600 sm:h-5 sm:w-5" />
              </div>
              <div>
                <h3 className="text-base font-semibold text-gray-900 sm:text-lg">
                  Rate Request Details
            ...
            {selectedRequest && (
              <>
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 p-3 sm:p-4">
                    ...
                    <div className="space-y-2 text-xs sm:text-sm">
                      <div className="flex items-center gap-2">
                        <User
                          size={12}
                          className="flex-shrink-0 text-gray-400 sm:h-3.5 sm:w-3.5"
                        />
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-3 sm:p-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-xs sm:grid-cols-3 sm:text-sm lg:grid-cols-4">
                    <div>
                      <span className="text-gray-500">Car Price:</span>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        $
                        {selectedRequest.calculationData?.depositAmount?.toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-500">Loan Amount:</span>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedRequest.calculationData?.loanTerm} years
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-500">Payment:</span>
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedRequest.calculationData?.interestRate}%
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="font-medium">
                        $
                        {selectedRequest.calculationData?.totalInterest?.toLocaleString()}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {selectedRequest.adminReply && (
                  <div className="rounded-lg border border-app-border bg-green-50 p-3 sm:p-4">
                    <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-center sm:gap-2">
                      <h4 className="text-app-bg text-sm font-medium sm:text-base">
                        Admin Reply
                      </h4>
                      {selectedRequest.repliedBy && (
                        <span className="text-app-bg text-xs">
                          by {selectedRequest.repliedBy}
                        </span>
                      )}
                    </div>
                    <p className="text-app-bg whitespace-pre-wrap text-xs sm:text-sm">
                      {selectedRequest.adminReply}
                    </p>
                  </div>
                )}
